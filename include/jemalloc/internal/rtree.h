...
#	define RTREE_NLIB get_lg_page_size()
#	define RTREE_NLIB_MIN MIN_LG_PAGE
#else /* DYNAMIC_PAGE_SIZE */
#	define RTREE_NLIB LG_PAGE
#	define RTREE_NLIB_MIN RTREE_NLIB
...
rtree_calc_leaf_maskbits(void) {
	assert(0 < rtree_levels[RTREE_HEIGHT - 1].bits);
	assert(0 < rtree_levels[RTREE_HEIGHT - 1].cumbits);
...
rtree_leaf_maskbits(void) {
	assert(0 < rtree_leaf_maskbits_value);
	return rtree_leaf_maskbits_value;
}

JEMALLOC_ALWAYS_INLINE uintptr_t
rtree_leafkey(uintptr_t key) {
	assert(0 < rtree_leafkey_mask_value);
...
rtree_subkey(uintptr_t key, unsigned level) {
	assert(0 < rtree_levels[level].bits);
	assert(0 < rtree_levels[level].cumbits);
	unsigned  ptrbits = ZU(1) << (LG_SIZEOF_PTR + 3);
